...
def handle_message(message):
    ...
    if url.startswith('http://') or url.startswith('https://'):
        if is_valid_url(url):
            short_code = shorten_url(url, 'bot')
            ...
            bot.reply_to(message, "The URL is not valid or reachable. Please try another one.")
    else:
